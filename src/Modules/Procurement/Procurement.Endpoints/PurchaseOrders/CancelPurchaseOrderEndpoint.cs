...
public sealed class CancelPurchaseOrderEndpoint : EndpointBase<CancelPurchaseOrderRequest, PurchaseOrderDto>
{
    private readonly IMediator _mediator;

    public CancelPurchaseOrderEndpoint(IResultToHttpMapper resultMapper, IMediator mediator)
        : base(resultMapper)
    {
        _mediator = mediator;
    }
    ...
    public override void Configure()
    {
        ...
        Summary(s =>
        {
            s.Summary = "Cancel purchase order";
            s.Description = "Cancels the purchase order when allowed.";
    ...
    public override async Task HandleAsync(CancelPurchaseOrderRequest req, CancellationToken ct)
    {
        ...
        var command = new CancelPurchaseOrderCommand(req.Id);
