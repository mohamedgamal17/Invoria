...
namespace Invoria.CustomerManagement.Endpoints.Tests.ReportCustomerMetrics
{
    ...
    [TestFixture]
    public class GetCustomerMetricsEndpointTests : ReportCustomerMetricsEndpointTestFixture
    {
        [Test]
        public async Task Should_return_current_metrics_for_all_periods()
        {
            ...
            await ReportRepository.Add(new ReportCustomerMetricsEntity(todayStart, 10, ReportPeriod.Daily));
            await ReportRepository.Add(new ReportCustomerMetricsEntity(monthStart, 25, ReportPeriod.Monthly));
            await ReportRepository.Add(new ReportCustomerMetricsEntity(yearStart, 40, ReportPeriod.Yearly));
            await ReportRepository.Add(new ReportCustomerMetricsEntity(DateTimeOffset.MinValue, 100, ReportPeriod.AllTheTime));

            // Act
            var response = await Client.GetAsync("/report/customers/creation/overview");

            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.OK);

            var envelope = await response.Content.ReadFromJsonAsync<Envelope<ReportCustomerMetricsDto>>();
