...
namespace Invoria.Procurement.Endpoints.Tests.ReportPurchaseOrdersCompletedMetrics
{
    ...
    [TestFixture]
    public class GetPurchaseOrdersCompletedMetricsEndpointTests : ReportPurchaseOrdersCompletedMetricsEndpointTestFixture
    {
        [Test]
        public async Task Should_return_current_metrics_for_all_periods()
        {
            ...
            await ReportRepository.Add(new ReportPurchaseOrdersCompletedMetricsEntity(todayStart, 10, ReportPeriod.Daily));
            await ReportRepository.Add(new ReportPurchaseOrdersCompletedMetricsEntity(monthStart, 25, ReportPeriod.Monthly));
            await ReportRepository.Add(new ReportPurchaseOrdersCompletedMetricsEntity(yearStart, 40, ReportPeriod.Yearly));
            await ReportRepository.Add(new ReportPurchaseOrdersCompletedMetricsEntity(DateTimeOffset.MinValue, 100, ReportPeriod.AllTheTime));

            // Act
            var response = await Client.GetAsync("/report/purchase-orders/completion/overview");

            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.OK);

            var envelope = await response.Content.ReadFromJsonAsync<Envelope<ReportPurchaseOrdersCompletedMetricsDto>>();
