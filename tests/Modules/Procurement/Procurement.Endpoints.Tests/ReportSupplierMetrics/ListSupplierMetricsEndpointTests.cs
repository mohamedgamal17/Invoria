...
namespace Invoria.Procurement.Endpoints.Tests.ReportSupplierMetrics
{
    ...
    [TestFixture]
    public class ListSupplierMetricsEndpointTests : ReportSupplierMetricsEndpointTestFixture
    {
        [Test]
        public async Task Should_return_paged_metrics_ordered_by_date_descending()
        {
            ...
            await ReportRepository.Add(new ReportSupplierMetricsEntity(today, 5, ReportPeriod.Daily));
            await ReportRepository.Add(new ReportSupplierMetricsEntity(yesterday, 7, ReportPeriod.Daily));
            await ReportRepository.Add(new ReportSupplierMetricsEntity(twoDaysAgo, 3, ReportPeriod.Daily));
            await ReportRepository.Add(new ReportSupplierMetricsEntity(today, 9, ReportPeriod.Monthly));

            // Act
            var response = await Client.GetAsync("/report/suppliers/creation/metrics?Period=Daily&Skip=0&Length=10");

            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.OK);

            var envelope = await response.Content.ReadFromJsonAsync<Envelope<PagingDto<ReportSupplierMetricsPeriodDto>>>();
        ...
        public async Task Should_respect_skip_and_length_for_paging()
        {
            ...
            for (var i = 0; i < 5; i++)
            {
                await ReportRepository.Add(new ReportSupplierMetricsEntity(
                    DateTimeOffset.Now.Date.AddDays(-i), i + 1, ReportPeriod.Daily));
            }

            // Act
            var response = await Client.GetAsync("/report/suppliers/creation/metrics?Period=Daily&Skip=1&Length=2");
