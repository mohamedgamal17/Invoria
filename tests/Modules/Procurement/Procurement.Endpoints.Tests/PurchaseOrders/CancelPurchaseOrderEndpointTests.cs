...
[TestFixture]
public class CancelPurchaseOrderEndpointTests : ProcurementTestFixture
{
    [Test]
    ...
    [Test]
    public async Task Should_return_domain_error_envelope_when_purchase_order_cannot_be_cancelled()
    {
        var purchaseOrderId = await CreateCompletedPurchaseOrderAsync();
    ...
    private async Task<string> CreateCompletedPurchaseOrderAsync()
    {
        var purchaseOrderId = await CreateAndSubmitPurchaseOrderAsync();
    ...
    private async Task<string> CreateAndSubmitPurchaseOrderAsync()
    {
        var createSupplierRequest = new CreateSupplierRequest
        {
            ...
            Name = "Supplier for cancel purchase order",
            ContactEmail = "cancel-supplier@example.com",
            Phone = "+1"
        };
        ...
        var purchaseOrderId = createEnvelope!.Result!.Id!;
