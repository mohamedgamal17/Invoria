...
[TestFixture]
public class CompletePurchaseOrderEndpointTests : ProcurementTestFixture
{
    [Test]
    public async Task Should_complete_purchase_order_when_it_is_approved()
    {
        ...
        envelope.Result!.State.Should().Be(Contracts.PurchaseOrders.PurchaseState.Completed);
    }

    [Test]
    public async Task Should_return_domain_error_envelope_when_purchase_order_cannot_be_completed()
    {
    ...
    private async Task<string> CreateDraftPurchaseOrderAsync()
    {
        var createSupplierRequest = new CreateSupplierRequest
        {
            ...
            Name = "Draft supplier for complete purchase order",
            ContactEmail = "complete-draft-supplier@example.com",
            Phone = "+1"
        };
