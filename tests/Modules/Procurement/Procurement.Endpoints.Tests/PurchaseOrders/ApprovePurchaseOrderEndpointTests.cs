...
[TestFixture]
public class ApprovePurchaseOrderEndpointTests : ProcurementTestFixture
{
    [Test]
    ...
    [Test]
    public async Task Should_return_domain_error_envelope_when_purchase_order_cannot_be_approved()
    {
    ...
    private async Task<string> CreateAndSubmitPurchaseOrderAsync()
    {
        var createSupplierRequest = new CreateSupplierRequest
        {
            ...
            Name = "Supplier for approve purchase order",
            ContactEmail = "approve-supplier@example.com",
            Phone = "+1"
        };
    ...
    private async Task<string> CreateDraftPurchaseOrderAsync()
    {
        ...
        {
            SupplierCode = "SUP-" + Guid.NewGuid().ToString("N")[..8],
            Name = "Draft supplier for approve purchase order",
            ContactEmail = "approve-draft-supplier@example.com",
            Phone = "+1"
        };
        var supplierResponse = await Client.PostAsJsonAsync("/suppliers", createSupplierRequest);
        supplierResponse.EnsureSuccessStatusCode();
