...
[TestFixture]
public class ReviseOrderCommandHandlerTests : OrderTestFixture
{
    ...
    [Test]
    public async Task ReviseOrder_sets_revision_and_preserves_allocation_when_order_is_processing()
    {
        await using var scope = ServiceProvider.CreateAsyncScope();
        ...
        (await Mediator.Send(new ReviseOrderCommand(order.Id!))).IsSuccess.Should().BeTrue();
        ...
        persisted.Status.Should().Be(OrderStatus.Revision);
