...
[TestFixture]
public sealed class OrderEntityUpdatedDomainEventHandlerTests
{
    ...
    [Test]
    public async Task Publishes_OrderUpdatedIntegrationEvent()
    {
        var bus = new Mock<IBus>();
        bus.Setup(b => b.Publish(It.IsAny<object>(), It.IsAny<Dictionary<string, string>>()))
            .Returns(Task.CompletedTask);

        var handler = new OrderEntityUpdatedDomainEventHandler(
            bus.Object,
            Mock.Of<Microsoft.Extensions.Logging.ILogger<OrderEntityUpdatedDomainEventHandler>>());
        ...
        var ev = new EntityUpdatedDomainEvent<Order, string>(order);

        await handler.Handle(ev, CancellationToken.None);

        bus.Verify(
            b => b.Publish(
                It.Is<OrderUpdatedIntegrationEvent>(e =>
                    e.Order.Id == "o1" &&
