...
public class OrderSagaTests
{
    ...
    public void Deliver_OrderRevisionRequested_after_allocation_succeeded_publishes_release_allocation_event()
    {
        ...
        bus.Verify(
            b => b.Publish(
                It.Is<ReleaseAllocationIntegrationEvent>(e =>
    ...
    [Test]
    public void Deliver_OrderCompletedIntegrationEvent_publishes_record_order_completed_metrics_saga_activity()
    {
        var bus = CreateBus();
        using var fixture = SagaFixture.For(() => new OrderSaga(bus.Object));
        ...
        bus.Verify(
            b => b.Publish(
                It.Is<RecordOrderCompletedMetricsSagaActivity>(a =>
                    a.OccurredOn == occurredOn),
                It.IsAny<Dictionary<string, string>>()),
            Times.Once);
    }
    ...
    [Test]
    public void Deliver_OrderCompletedIntegrationEvent_publishes_request_order_allocation_when_allocation_exists()
    {
        var bus = CreateBus();
        using var fixture = SagaFixture.For(() => new OrderSaga(bus.Object));
        ...
        fixture.Deliver(BuildOrderCompleted("order-1", "alloc-1", [], hasBillableItems: false));

        fixture.HandlerExceptions.Should().BeEmpty();

        bus.Verify(
            b => b.Publish(
                It.Is<RequestOrderAllocationIntegrationEvent>(e =>
                    e.OrderId == "order-1" &&
                    e.AllocationId == "alloc-1"),
                It.IsAny<Dictionary<string, string>>()),
            Times.Once);
    ...
    [Test]
    public void Deliver_OrderCompletedIntegrationEvent_does_not_publish_request_order_allocation_when_no_allocation()
    {
        var bus = CreateBus();
        using var fixture = SagaFixture.For(() => new OrderSaga(bus.Object));
        ...
        bus.Verify(
            b => b.Publish(
                It.IsAny<RequestOrderAllocationIntegrationEvent>(),
                It.IsAny<Dictionary<string, string>>()),
            Times.Never);
    }
