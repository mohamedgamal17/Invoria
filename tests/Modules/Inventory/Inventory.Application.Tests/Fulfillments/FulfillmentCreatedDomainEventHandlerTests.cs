...
[TestFixture]
public class FulfillmentCreatedDomainEventHandlerTests
{
    [Test]
    public async Task Publishes_FulfillmentCreatedIntegrationEvent_with_fulfillment_order_and_allocation_ids()
    {
        var bus = new Mock<IBus>();
        bus.Setup(b => b.Publish(It.IsAny<object>(), It.IsAny<Dictionary<string, string>>()))
            .Returns(Task.CompletedTask);

        var handler = new FulfillmentCreatedDomainEventHandler(
            bus.Object,
            Mock.Of<Microsoft.Extensions.Logging.ILogger<FulfillmentCreatedDomainEventHandler>>());

        var allocation = CreateFullyAllocatedAllocation("order-1");
        var fulfillment = Fulfillment.CreateFromAllocation(allocation);
        var ev = fulfillment.DomainEvents.OfType<FulfillmentCreatedDomainEvent>().Single();

        await handler.Handle(ev, CancellationToken.None);

        bus.Verify(
            b => b.Publish(
                It.Is<FulfillmentCreatedIntegrationEvent>(msg =>
