...
[TestFixture]
public class RequestDispatchDomainEventHandlerTests
{
    [Test]
    public async Task Publishes_DispatchFulfillmentIntegrationEvent_with_fulfillment_order_and_allocation_ids()
    {
        var bus = new Mock<IBus>();
        bus.Setup(b => b.Publish(It.IsAny<object>(), It.IsAny<Dictionary<string, string>>()))
            .Returns(Task.CompletedTask);

        var handler = new RequestDispatchDomainEventHandler(
            bus.Object,
            Mock.Of<Microsoft.Extensions.Logging.ILogger<RequestDispatchDomainEventHandler>>());
        ...
        var ev = fulfillment.DomainEvents.OfType<RequestDispatchDomainEvent>().Single();

        await handler.Handle(ev, CancellationToken.None);

        bus.Verify(
            b => b.Publish(
                It.Is<DispatchFulfillmentIntegrationEvent>(msg =>
